...
ROWS = [
    ("host",),
    ("kv", "Now",   "VP @ Cyber Security Study Club, UPNVJ"),
    ("kv", "Also",  "Lab Assistant @ CS Lab, UPNVJ"),
    ("kv", "Also",  "Penetration Tester @ Reunusa"),
    ("kv", "Edu",   "S.Kom Informatics, UPN Veteran Jakarta"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Security", "Burp Suite, Suricata, Wireshark, ZAP"),
    ("kv", "Crypto",   "ML-KEM-768, AES-256-GCM, FIPS 203"),
    ("kv", "Systems",  "C, C++, x86 Asm (FASM), Linux, Shell"),
    ("kv", "Other",    "Python, Java, SQL, Git, KVM, GTK4"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Post-quantum file transfer (ML-KEM-768)"),
    ("bul", "IDPS: 100% SQL injection prevention rate"),
    ("bul", "Real-time honeypot dashboard (zero-dep)"),
    ("bul", "BadUSB scareware PoC in x86 Assembly"),
]
...
parts = [
    ...
    f'<rect x="0.5" y="0.5" width="{W-1}" height="{H-1}" rx="12" fill="none" stroke="{FRAME}" stroke-opacity="0.6"/>',
    f'<line x1="0" y1="{TITLEBAR_H}" x2="{W}" y2="{TITLEBAR_H}" stroke="{FRAME}" stroke-opacity="0.4"/>',
]
for i, dotcol in enumerate(["#ff5f56", "#ffbd2e", "#27c93f"]):
    parts.append(f'<circle cx="{PAD + i*16}" cy="{TITLEBAR_H/2}" r="5" fill="{dotcol}"/>')
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">moemairu@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">moemairu</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">github</tspan></text>'
                 f'<line x1="{KEY_X+144}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
